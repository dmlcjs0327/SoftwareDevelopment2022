...
class Tello11111Sensor(Sensor):
    ...
    def __func_sensor(self):
        ...
        try:
            ...
            while not self.__stop_event.is_set():
                packet_data = self.take_data_from_sensor()
                self.change_data_to_info(packet_data)
    ...
    def change_data_to_info(self, packet_data: bytes):
        """
        data를 Planner가 이해할 수 있는 info로 변경한다
        """
        if len(packet_data) != 1460 and packet_data is not None: # frame의 끝이 아니면,
